...
{
    public class GetPropertyByCodeQueryHandlerTest
    {
        private readonly DbContextOptions<IslaNovaContext> _dbOptions;
        private readonly IMapper _mapper;

        public GetPropertyByCodeQueryHandlerTest()
        {
        ...
        public async Task Handle_Should_Return_Property_When_Exists()
        {
            ...
            var handler = new GetPropertyByCodeQueryHandler(repository, authServiceMock.Object, _mapper);

            // Act
            var result = await handler.Handle(new GetPropertyByCodeQuery() { Code = "000123" }, CancellationToken.None);
        ...
        public async Task Handle_Should_Return_Null_When_Property_Not_Found()
        {
            ...
            // Act
            var result = await handler.Handle(new GetPropertyByCodeQuery() { Code = "000000" }, CancellationToken.None);
